...
bool CheckMatrix(std::string matrix[3][3]) {
    int x = 0;
    int o = 0;
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
...
bool CheckRows(std::string matrix[3][3]) {
    int xRow = 0;
    int oRow = 0;
    int xCol = 0;
    int oCol = 0;
    for (int i = 0; i < 3; i++) {
        ...
        if (matrix[0][i] == "X" && matrix[0][i] == matrix[1][i] && matrix[0][i] == matrix[2][i]) {
            xCol++;
        } else if (matrix[0][i] == "O" && matrix[0][i] == matrix[1][i] && matrix[0][i] == matrix[2][i]) {
            oCol++;
        }
    }
    if ((xRow + oRow) > 1 || (xCol + oCol) > 1) {
        return false;
    } else {
        return true;
    }
